  ...
  try {
    switch (type) {
      ...
      case 'setBalance': {
        ...
        if (typeof newBalance !== 'number' || !Number.isFinite(newBalance) || newBalance < 0 || newBalance > 1_000_000_000) {
          return c.json({ error: 'Invalid balance (must be a finite non-negative number up to 1,000,000,000)' }, 400);
        }
